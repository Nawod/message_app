...
let initialState : MessageStates = {
...
const messageSlice = createSlice({
    ...
    reducers: {
        ...
        deleteMessage(state,action) {
            function deleteMessageRecursively(messages: MessageItem[], id: string): MessageItem[] {
                return messages
            ...
            let tempList = deleteMessageRecursively(state.messages, action.payload);
        ...
        editMessage(state,action) {
            function updateMessage(messages: MessageItem[], id: string, newMessage: string): MessageItem[] {
            ...
            let tempList = updateMessage(state.messages, action.payload.id, action.payload.message);
            state.messages = tempList;
        ...
        sendReply(state,action) {
            function updateMessageList(messages: MessageItem[], id: string, newReply: MessageItem): MessageItem[] {
            ...
            let tempList = updateMessageList(state.messages, action.payload.messageId, action.payload.message);
        ...
        changeMessagePriority(state,action) {
            function updateMessagePriority(messages: MessageItem[], id: string,isPlus : boolean): MessageItem[] {
                return messages.map((message : MessageItem) => {
                  if (message.id === id) {
            ...
            let tempList = updateMessagePriority(state.messages, action.payload.messageId, action.payload.isPlus);
